...
class AdaptadorComisiones(ContratoProyecto):
    ...
    def validar_parametros(self, parametros: Dict[str, Any]) -> List[str]:
        ...
        configurado = self._periodo_configurado()
        if mes is not None and configurado["mes"] is not None and mes != configurado["mes"]:
            errores.append(
                f"El mes seleccionado ({mes}) no coincide con el mes configurado "
                f"en Comisiones ({configurado['mes']}). Actualice primero la "
                "configuración estable del proyecto."
            )

        if anio is not None and configurado["anio"] is not None and anio != configurado["anio"]:
            errores.append(
                f"El año seleccionado ({anio}) no coincide con el año configurado "
                f"en Comisiones ({configurado['anio']})."
            )
    ...
    def _comando(self, anio: int) -> List[str]:
        codigo = (
            "import generar_informe_final as g; "
            f"g.ejecutar(anio_proceso={anio})"
        )
