...
class VentanaPrincipal(tk.Tk):
    ...
    def _mostrar_inicio(self):
        self._limpiar_panel()
        contenedor = tk.Frame(self.panel_principal, bg=COLORES["panel"])
        contenedor.pack(fill="both", expand=True, padx=38, pady=38)
        ...
        tk.Label(
            contenedor,
            text=(
                "Cada módulo declara sus parámetros, validaciones, reglas de seguridad "
                "y resultados sin mezclar la lógica de otros proyectos."
            ),
            wraplength=720,
            justify="left",
            bg=COLORES["panel"],
        ...
        tarjetas = tk.Frame(contenedor, bg=COLORES["panel"])
        tarjetas.pack(fill="x")
        self._tarjeta_resumen(
            tarjetas,
            "Comisiones 250115",
            "Informe informativo de contabilidad, facturas, homologaciones y conciliación.",
            COLORES["azul"],
        ).pack(fill="x", pady=6)
        self._tarjeta_resumen(
            tarjetas,
            "Inversiones",
            "Integración preparada para ejecución local protegida. Publicación corporativa bloqueada.",
            COLORES["violeta"],
        ).pack(fill="x", pady=6)
    ...
    def _tarjeta_resumen(self, padre, titulo, descripcion, color):
        ...
        barra = tk.Frame(tarjeta, bg=color, width=6)
        barra.pack(side="left", fill="y")
        contenido = tk.Frame(tarjeta, bg="#F8FAFC")
